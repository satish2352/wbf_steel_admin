...
const validateImageSize = (file) => {
  return new Promise((resolve, reject) => {
    ...
    if (file.size > maxSize) {
      reject(`Image ${file.name} must be less than 2MB`);
      return;
    }
    ...
    const img = new Image();
    img.onload = () => {
      if (img.width === 450 && img.height === 500) {
        resolve(true);
      } else {
        reject(
          `Image must be exactly 450px (width) × 500px (height)`
        );
      }
    };

    img.onerror = () => reject(`Could not load image: ${file.name}`);
...
const handleImageChange = async (e) => {
  ...
  for (const file of files) {
    try {
      await validateImageSize(file);  // validate dimensions
  ...
  // Show preview of valid images
  const previews = validFiles.map((file) => URL.createObjectURL(file));
  setImagePreviews(previews);

  // Show validation errors
  if (invalidFiles.length > 0) {
    setErrors({ img: invalidFiles });
  } else {
    setErrors({ img: null });
  }
};
                  ...
                  <Row>
                    ...
                    <Col md={12} className="mt-3">
                      ...
                      <Form.Group controlId="project_images">
                        <Form.Label>Upload multiple project Images<span className="text-danger">*</span> <small className="text-danger">(Image size must be less than 2MB & Image must be 450x500 pixels)</small></Form.Label>
                        <Form.Control
                          type="file"
                          name="project_images"
                          multiple
                          ...
                          errors.img && (
                            <Form.Control.Feedback type="invalid">
                              {Array.isArray(errors.img)
                                ? errors.img.map((err, idx) => <div key={idx}>{err}</div>)
                                : <div>{errors.img}</div>
                              }
